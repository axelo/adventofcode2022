...
int main(void) {
    ...
    while (fgets(line, LINE_CAP, stdin)) {
        ...
        case MOVES: {
            ...
            assert(found == 3 && "Expected move syntax");

            int from_stack_start_index = n_crates[from_stack - 1] - amount;
            assert(from_stack_start_index >= 0 && "from_stack_start_index out of bounds");

            int to_stack_end_index = n_crates[to_stack - 1];
            assert(to_stack_end_index >= 0 && (to_stack_end_index + amount) < CRATES_CAP && "to_stack_end_index out of bounds");

            for (int i = 0; i < amount; ++i) {
                char crate = stacks[from_stack - 1][from_stack_start_index + i];
                stacks[to_stack - 1][to_stack_end_index + i] = crate;
            }

            n_crates[from_stack - 1] -= amount;
            n_crates[to_stack - 1] += amount;
        } break;
        }
    }
