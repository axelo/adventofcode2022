...
int main(void) {
    ...
    int sizes[NODE_CAP];
    int n_sizes = 0;
    int root_size = find_total_sizes(0, nodes, &n_sizes, sizes);
    int unused_space = 70000000 - root_size;
    int space_missing = 30000000 - unused_space;
    assert(space_missing > 0);

    int smallest_dir = root_size;

    for (int i = 0; i < n_sizes; ++i) {
        if (sizes[i] >= space_missing && sizes[i] < smallest_dir) {
            smallest_dir = sizes[i];
        }
    }
